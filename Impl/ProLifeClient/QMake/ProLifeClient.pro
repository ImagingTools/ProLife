...
LIBS += -lprolifeqml -lprolifestyle -lImtCoreLoc -lProLifeLoc
...
win32-msvc*{
	...
	# copying all Qt DLLs to destination directory
        greaterThan(QT_MAJOR_VERSION, 4): QMAKE_POST_LINK = set path=$(QTDIR)\bin;%path% && $(QTDIR)\bin\windeployqt --qmldir=$(IMTCOREDIR)/Qml --qmldir=$(PROLIFEDIR)/Include/prolifeqml/Qml $$DESTDIR
}
