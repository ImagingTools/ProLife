...
| `02 GetToken and GetTokenList` | success shape incl. `tokenHash` exposure, missing/empty `id`/`userId`, not-found, unrelated-userId is rejected as not authorized (ownership enforced) |
| `03 ValidateToken` | success, missing/empty `token`, garbage token, past-expiry token, malformed-expiry token |
| `04 RevokeToken` | success, `GetToken`/`ValidateToken` reflect revocation, idempotent double-revoke of a still-existing token, missing/empty `id`, nonexistent `id` rejected as not-found (see "Ownership enforcement" below), immediate 403 on first PAT-auth use after revoke |
| `05 DeleteToken` | delete-after-revoke, not-found afterwards, double-delete / never-existed `id` now rejected as not-found rather than idempotent (see "Ownership enforcement" below), missing/empty `id` |
...
| `07 Ownership and Cross-User Access` | source-confirmed: all six PAT resolvers now check the caller's identity via `CPersonalAccessTokenControllerComp::IsCallerAuthorizedForUser` - a non-admin, non-owning caller can no longer list/read/revoke/delete or create PATs for another `userId`; not-found and not-authorized are reported identically so a caller can't enumerate other users' token ids |
...
newman run PatTokenApi.postman_collection.json -e PatTokenApi-Dev.postman_environment.json `
  ...
  --folder "07 Ownership and Cross-User Access (enforced, source-confirmed)" `
...
- Ownership enforcement is a real, current, source-confirmed property of this API (folder `07`) as of ImtCore commit `6dd1ef2568` ("Close PAT authorization gap and fix GQL request-context loss on forwarded requests", 2026-07-14): `CPersonalAccessTokenControllerComp::IsCallerAuthorizedForUser` gates `CreateToken`/`GetToken`/`GetTokenList`/`RevokeToken`/`DeleteToken` to the token's own owner or an admin caller, and unauthorized/not-found are reported with the identical message so a caller can't use the API to enumerate other users' token ids. Before that commit this suite documented the opposite (no enforcement at all) - if folder `07` starts failing again after a future change, that's a real regression of this gate, not a suite bug.
...
## Server bugs found by this suite (fixed in ImtCore source, verified against a rebuilt server)

Running this suite against a live server surfaced real, confirmed server-side bugs across two rounds, since fixed at the source level in ImtCore and verified passing (0 failures / 658 assertions) against a Debug server rebuilt from current source:

1. **`RevokeToken` rejected re-revoking an already-revoked (or expired) token.** `CPersonalAccessTokenManagerComp::RevokeToken` (`Include\imtauth\CPersonalAccessTokenManagerComp.cpp`) checked `tokenPtr->IsValid()` (`!IsRevoked() && !IsExpired()`) as a *precondition* for revoking - but a token's very purpose in being revoked is to become invalid, so this made the operation fail with `"Invalid token object"` (surfaced as `"Failed to revoke token with id '<id>'"`) on any second revoke attempt, inconsistent with `DeleteToken`'s idempotent behavior (see below) and with what a revoke operation should do. Fixed by replacing the `IsValid()` check with a plain null-pointer guard (the check was also an unguarded null-dereference risk before the fix, since `dynamic_cast` can return `nullptr`). Folder `04`'s double-revoke-of-a-still-existing-token case exercises this.
2. **`DeleteToken` was (correctly) idempotent at the manager layer - our own initial test expectations were wrong, not the server.** `CPersonalAccessTokenManagerComp::DeleteToken` just forwards to the collection's generic `RemoveElements()`, which succeeds even for an id that never existed or was already deleted. This was previously an explicitly untested/commented-out case in `Tests\PersonalAccessTokenTest\CPersonalAccessTokenTest.cpp`. **Superseded by fix #3 below**: the GQL controller layer now looks the token up (to check ownership) *before* reaching the manager's idempotent `DeleteToken`/`RevokeToken`, so a truly nonexistent (or already-deleted) id is now rejected with `"Token with id '<id>' not found"` at the controller layer instead of ever reaching the manager's idempotent path - see folder `05`'s `double-delete`/`nonexistent id` cases, both now asserting rejection rather than idempotent `success:true`.
3. **No caller-identity check on any of the six PAT resolvers - any authenticated user could list/read/revoke/delete and even create PATs for any other `userId`.** Fixed by ImtCore commit `6dd1ef2568` ("Close PAT authorization gap and fix GQL request-context loss on forwarded requests", 2026-07-14), which added `CPersonalAccessTokenControllerComp::IsCallerAuthorizedForUser` (self-or-admin) as a precondition on every resolver, with not-found and not-authorized reported identically to prevent token-id enumeration. This is what folder `07` was originally built to document as a live gap; it now documents the fix instead (folder renamed from "no enforcement" to "enforced"). This is also the source of change #2 above, since the ownership check requires looking the token up first.

Also verified still working: the PAT scope-intersection fix made earlier to `CUserBaseInfo::GetPermissions`/`CAuthenticationManagerComp::CreateGqlContext` (`06`'s scope-intersection probes would return an empty permission set for every scenario against a pre-fix binary, even where the user's role genuinely grants the permission - confirmed via the `Authorization` login response itself reporting the correct `permissions` string, isolating the gap to the PAT auth-context path rather than role/permission resolution in general).
