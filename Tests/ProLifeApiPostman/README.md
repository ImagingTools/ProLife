...
2. `01 Accounts` -> `02 Orders` (includes the order/product lifecycle scenario, see below) -> `03 Devices` -> `04 Software Products - Licenses` -> `05 Device Document Service` -> `06 Workspace Dashboards` (read-only, run last so it sees seeded data).
3. `07 Multi-role Scenario - Pages, Commands, Profile` - creates 3 roles + 3 users from scratch (Full Access / Accounts Viewer / No Access, see below), switches the active `token` between them, and asserts `PagesData`/`GetCommands`/`GetProfile` visibility differs correctly per permission set. Re-authenticates as `su` at the end and covers the remaining app-level infra fields (`GetSettings`, `SetSettings`, `GetStyleData`, `GetWebSocketUrl`, `Search`, `GetProductPermissions`).
4. `08 Meta Info Propagation - Account & Order Field Sync` - verifies that renaming an Account, or changing an Order's delivery/purchase id, is immediately reflected in `DevicesList`/`SoftwareProductsList`/`OrdersList` for everything linked to them (see below).
5. `99 Cleanup` - best-effort removal of everything created (including the roles/users from folder `07` and the fixtures from folder `08`); CI mainly relies on the full DB reset instead.

## Order/Product Lifecycle Scenario (folder `02 Orders`)

Beyond plain `OrderAdd`/`OrderUpdate`/`OrdersList`/`OrderItem` CRUD, this folder also verifies the product-add/remove side effects on `Devices`/`SoftwareProducts` that `Tests\backend\postman_collection.json`'s `Modification Tests/Superuser/Orders` folder already covered for the legacy suite - traced against the live source (`COrderCollectionControllerComp.cpp`) rather than assumed:

- **Adding a new (`isNew: true`) `orderProducts` line auto-creates a real entity.** A `Software` line calls `CreateNewSoftware` (creates a `SoftwareProduct` with `orderUuid` set to the order); a `Hardware` line calls `CreateNewHardware` (creates a `Device` with `orderId` set). Verified by `OrderUpdate (Add new software product)` -> `SoftwareProductItem (Check new software linked to order)`, and `OrderUpdate (Replace with new hardware product)` -> `DeviceItem (Check new device linked to order)`.
- **A new device with both `macAddress` and `serialNumber` set is auto-marked `productionStatus: Finished`** (`CreateNewHardware`'s own logic, not a separate step) - verified in the same `DeviceItem` check above. This is the same mechanism `03 Devices`' plain `DeviceAdd` request relies on to get a `Finished` device without the server ever doing so automatically on its own.
- **Removing a product from the order (by omitting it from a subsequent `OrderUpdate`'s `orderProducts`) clears that entity's order link back to empty.** `COrderCollectionControllerComp::GenerateDifferences` diffs old vs. new `orderProducts` by id; anything dropped gets `UpdateOrderForHardware`/`UpdateOrderForSoftware(id, "")`. Verified twice: once for the software product dropped when the hardware line replaces it (`SoftwareProductItem (Check Order-ID cleared after replacement)`), and again for that same hardware device when it's later dropped in favour of pre-existing products (`DeviceItem (Check previously-added device now cleared)`) - confirming the clear-on-removal behaviour isn't a one-off fluke of the first case.
- **Linking an already-existing (`isNew: false`) product by id** (as opposed to creating a new one) also sets its order link, without recreating the entity - verified via two dedicated standalone fixtures (`Setup: DeviceAdd`/`Setup: SoftwareProductAdd (... for order re-link test)`, isolated from the shared `deviceId`/`softwareProductId` env vars used elsewhere in the collection to avoid cross-test interference) followed by `OrderUpdate (Link pre-existing products)` -> `DeviceItem`/`SoftwareProductItem (Check pre-existing ... now linked)`.

## Meta Info Propagation Scenario (folder `08`)

Covers the same behaviour `Tests\backend\postman_collection.json`'s `Updating meta info Tests` folder exercised for the legacy suite - a customer's own Account name, and an Order's delivery id (`orderId`) / purchase id (`purchaseId`), are never copied onto a linked `Device`/`SoftwareProduct`/`Order` at link time. They're resolved **live**, on every list query, by `CDeviceMetaInfoCreatorComp`/`CSoftwareMetaInfoCreatorComp`/`COrderMetaInfoCreatorComp` (each looks up the linked Order/Account fresh via `m_orderCollectionCompPtr`/`m_accountCollectionCompPtr` and writes the *current* name/id into the response's `customerLink`/`deliveryIdLink`/`purchaseIdLink` `ObjectLink` fields - `Sdl\prolife\1.0\Sensors.sdl`/`Licenses.sdl`'s list-item types, not the flat `DeviceData`/`SoftwareProductData` single-get types, which don't carry these fields at all). Traced against the live source rather than assumed - so a regression that broke this live-resolution (e.g. an accidental cache) would be caught by this scenario, not just a copy-paste-stale-data bug.

Sequence: create a standalone Account + Order + Device + SoftwareProduct (the latter two linked directly via their own settable `orderId`/`orderUuid` fields, not via `OrderUpdate`'s `orderProducts` mechanism, since only the meta-info read path is under test) -> confirm `DevicesList`/`SoftwareProductsList`/`OrdersList` show the account's current name and the order's current delivery/purchase id -> rename the Account -> re-check all three reflect the new name -> change the Order's delivery/purchase id -> re-check `DevicesList`/`SoftwareProductsList` reflect the new ids (per the user's request, `OrdersList` isn't re-checked for this part - its own `orderId`/`purchaseId` fields are read directly off the order's own record, not resolved via propagated meta-info, so there's nothing to propagate there). List queries use `filterModel: { sortingInfo: [{ fieldId: "timeStamp", sortingOrder: "DESC" }] }` with a small `count` to reliably surface the just-created/-updated item first without needing an id-based filter, mirroring the legacy suite's own approach.

**Pitfall found and fixed while writing this**: an `OrderUpdate` call that changes only the order's own `orderId`/`purchaseId` fields (not touching product links at all) must still explicitly re-list every currently-linked product in `orderProducts` (with `isNew: false`). `COrderCollectionControllerComp::GenerateDifferences` diffs against the order's *live* current-products collection (which includes anything linked via a direct `orderId`/`orderUuid` write, not just what a previous `OrderUpdate` call's `orderProducts` happened to contain) - passing `orderProducts: []` on such a call silently drops every linked product's order reference as an unintended side effect (confirmed empirically: an earlier draft of `OrderUpdate (Meta Info scenario - change delivery/purchase id)` did exactly this, and the following `DevicesList`/`SoftwareProductsList` checks caught it immediately - deliveryId/purchaseId came back empty instead of updated). This is worth knowing for any real client code too: a naive "patch just these two fields" Order update can silently unlink every device/software on the order if it doesn't preserve the full current product list.
