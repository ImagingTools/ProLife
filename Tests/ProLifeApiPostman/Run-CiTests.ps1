...
<#
.SYNOPSIS
    CI entry point for the ProLife Api Postman suite: resets the
...
.PARAMETER HttpPort
    ...
    HttpPort attribute, currently 17778).
...
function Reset-TestDatabase {
    ...
    # already exist - mirrors Puma's PumaServerPgTest.exe behavior - so
    # there's nothing else to do here beyond dropping a stray leftover DB.
