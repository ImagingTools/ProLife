...
//
// Split out from administration.test.js: these open document tabs via the same server-side
// Collection Document Service used by Devices/Orders/Accounts/Software editors (workspace keyed by
// userId only - see ImtCore/Include/imtservergql/CCollectionDocumentServiceControllerComp.cpp). Like
// every spec here it runs as its own user (`administrationEditor`, see fixtures/users.js), so no
// other file's navigation can close a tab this one is mid-test on.
...
test.describe('Administration / editors', () => {
  ...
  test.describe.serial('Role editor', () => {
    ...
    // both, matching the same-mechanism consolidation used across the other editor suites. Every name
    // typed in this file is fixed, not timestamped: screenshots capture them, and a per-run value never
    // matches its baseline. Run-CiTests.ps1 restores the database before every run and the Remove tests
    // delete the records again, so no earlier run's duplicate is there for Edit/Remove to trip over.
    test('fill fields and save', { tag: '@mutating' }, async () => {
      createdRoleName = 'ProLifeGui Test Role';
    ...
    test('edit existing role and save', { tag: '@mutating' }, async () => {
      ...
      const editedDescription = 'Edited role description';
  ...
  test.describe.serial('User editor', () => {
    ...
    test.beforeAll(async ({ browser }, testInfo) => {
      ...
      createdUsername = 'prolifegui_test_user';
    });
  ...
  test.describe.serial('Group editor', () => {
    ...
    // Fixed name - see the note on the Role editor's 'fill fields and save'.
    test('fill fields and save', { tag: '@mutating' }, async () => {
      createdGroupName = 'ProLifeGui Test Group';
    ...
    test('edit existing group and save', { tag: '@mutating' }, async () => {
      ...
      const editedDescription = 'Edited group description';
      await editor.setDescription(editedDescription);
