// Hardware (Devices) EDITOR - full functional coverage.
//
...
// interaction bodies; field editability is permission-driven (DeviceEditor.checkPermissions), which
// the screenshots + the structural "editable fields" test record.
//
...
test.describe('Hardware / editor', () => {
  ...
  test.describe.serial('new document', () => {
    ...
    test('save blocked - missing required fields', async () => {
      // Touch a field the validator does NOT check (Description), leaving Device Type/Configuration
      // unset, so the refusal is about exactly those two.
      ...
      // The server refuses the save as well and says so in a toast. Wait for it so the shot always has
      // it, then wait out its 5s auto-close (PopupContainer.autoCloseInterval): it would otherwise
      // vanish at an arbitrary moment during the next test's shot.
      await gui.expectVisible(page, ['PopupMessage'], 'the server should report the refused save');
      await gui.checkScreenshot(page, 'device-editor-save-blocked-empty');
      // expectCount polls; expectHidden would assert "hidden right now" and fail on a toast mid-life.
      await gui.expectCount(page, ['PopupMessage'], 0, 'the toast should close by itself');
    });
    ...
    test('undo / redo', async () => {
      ...
      // A field commits on blur, not while typed into; otherwise the Undo click itself does the commit
      // and races the undo. Clicking the current page's nav item blurs and changes nothing else.
      await editor.openEditorPage('Production');
      await editor.undo();
